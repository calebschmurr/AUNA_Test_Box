    ...
    pin_list.clear_pin_list();
      ...
      while(inputArray[loc]!='!'){
        ...
        if(pin_type[0]=='I'){
          ...
          if(pin_num>=54 && pin_num<=69){
            pinMode(pin_num,INPUT);
            pin_list.add_pin(atoi(pin_num),PinAnalogInput);
          }else{
            pinMode(atoi(pin_num),INPUT_PULLUP);
            pin_list.add_pin(atoi(pin_num),PinDigitalInput);
          }
            if(Debug){
              Serial.print("Pin ");
              Serial.print(pin_num);
              ...
              pinMode(atoi(pin_num),OUTPUT);
             // add_pin(&activePinsListSize, activePinsList, atoi(pin_num),0);
              pin_list.add_pin(atoi(pin_num),PinOutput);
            if(Debug){
              Serial.print("Pin ");
              Serial.print(pin_num);
